...
<?else:?>
    <td<?if ($type[0]=="multiselectbox" || $type[0]=="textarea" || $type[0]=="statictext" ||
        ...
        if ($type[0]=="checkbox")
            ...
            echo $Option[1];
        if($type[0]!="limitedText" && $type[0]!="number"){
            if ($sup_text <> '')
            {
...
function renderInput($Option, $arControllerOption, $fieldName, $val)
{
    ...
    elseif($type[0]=="limitedText"):
        ?><textarea  class="req" <?if(isset($arControllerOption[$Option[0]]))echo ' disabled title="'.GetMessage("MAIN_ADMIN_SET_CONTROLLER_ALT").'"';?>  rows="10" cols="45"  maxlength="<?=$Option[4]?>" value="<?echo htmlspecialcharsbx($val)?>" name="<?=htmlspecialcharsbx($fieldName)?>"<?=$disabled?>><?echo htmlspecialcharsbx($val)?></textarea><?
    elseif($type[0]=="number"):
        ?><input type="text" class="ss req <?=$Option[6]?>" <?=$Option[6]? "" : " " ?>  inputmode="numeric"<?if(isset($arControllerOption[$Option[0]]))echo ' disabled title="'.GetMessage("MAIN_ADMIN_SET_CONTROLLER_ALT").'"';?> size="<?echo $type[1]?>"  maxlength="<?=$Option[4]?>" value="<?echo htmlspecialcharsbx($val)?>" name="<?=htmlspecialcharsbx($fieldName)?>"<?=$disabled?><?=($type[0]=="password" || $type["noautocomplete"]? ' autocomplete="new-password"':'')?>><?
